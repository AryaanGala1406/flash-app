...
const Dashboard = () => {
  ...
  const fetchFlashcards = async () => {
    try {
      const res = await axios.get('https://flash-app-three.vercel.app/api/flashcards');
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (editMode) {
        await axios.put(`https://flash-app-three.vercel.app/api/flashcards/${currentCard.id}`, form);
        setEditMode(false);
        setCurrentCard(null);
      } else {
        await axios.post('https://flash-app-three.vercel.app/api/flashcards', form);
      }
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://flash-app-three.vercel.app/api/flashcards/${id}`);
      fetchFlashcards();
    } catch (error) {
